#!/bin/zsh
# On-Prem / Hybrid Kafka Ingest - GCP Edition
# Same on-prem services as the original, but with GCP cloud references
# instead of AWS (e.g. svc_gcp_function instead of svc_aws_lambda,
# Cloud Interconnect instead of Smart Connect).
SCRIPT_DIR="${0:A:h}"
...
while true; do
  ...
  for ((i=1; i<=$LOGS_PER_REQUEST; i++)); do
    ...
    case $SVC in
      ...
      *active-directory*)
        MSG="[AD-DC-01] Kerberos TGT issued for user: svc_gcp_function" ;;
      *)
    ...
    # Kafka topic IDs for this team (pick one for this log, keep full list for context)
  ...
  clear
  echo "🏢 ON-PREM KAFKA INGEST (GCP) | MODE: $MODE | STATE: $STATE | BATCH SIZE: $LOGS_PER_REQUEST | HTTP: $HTTP_STATUS"
