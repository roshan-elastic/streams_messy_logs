...
Both scripts use the Elastic **Bulk API** to send data to [wired streams](https://www.elastic.co/docs/solutions/observability/streams/wired-streams) endpoints (`logs.otel` or `logs.ecs`), so you can use Streams to partition, process, and analyse the data.
...
Before sending data, you must enable wired streams in your deployment:
...
## Quick start: send data to your cluster
...
You can run one or both. Each script sends batches to the wired streams **Bulk API** every second. The default target is the `logs.otel` endpoint (OpenTelemetry-normalised format). To use ECS field names without transformation, pass `--preferred-schema ecs`:
...
| Flag | Description |
|------|-------------|
| `--mode failure` | Switch to a failure scenario (errors, timeouts, critical states). See [Failure scenario](#failure-scenario) below. |
...
That way, you turn raw “messy” logs into structured, queryable data that supports actionable insights and AI assistants.

---

## Failure scenario

You can simulate an incident by running the scripts with `--mode failure`:
...
The diagram below summarises this failure scenario:
...
2. **Inject failure**  
   Stop the scripts and restart them with `--mode failure`. Keep this running for a few minutes so there is a clear “during incident” window.
...
Using Streams (partitioning, processing) and the AI assistant on the same data helps show how structured logs and observability tools support detection, explanation, and proof of resolution.
